...
For a widget that fetches, that ladder runs: nothing under ~300ms → `Skeleton` (or `SpinnerContainer`, if the shape is irregular) on first load → `ProgressIndeterminate` on every refetch after that.
